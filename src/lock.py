...
def main():
    ...
    for i, n in enumerate(names, 1):
        print(f"  {i}. {n}")
    print("\nEnter the number or name of the identity to lock: ", end="")
    try:
        choice = input().strip()
    except EOFError:
    ...
    # Handle numeric input (user enters 1, 2, 3, etc.)
    if choice.isdigit():
        idx = int(choice) - 1  # Convert to 0-based index
        if 0 <= idx < len(names):
            lock_identity = names[idx]
        else:
            print(f"ERROR: Number '{choice}' is out of range. Choose 1-{len(names)}")
            return False
    # Handle name input (user enters exact name)
    elif choice in db:
        lock_identity = choice
    else:
        print(f"ERROR: '{choice}' not found. Enter a number (1-{len(names)}) or exact name from: {names}")
        return False
